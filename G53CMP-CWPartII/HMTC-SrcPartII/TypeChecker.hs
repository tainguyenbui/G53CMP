...
-- and the fact that the else-branch is optional taken care of.
    ecs' <- mapM (elsif env) ecs    		        -- env |- (es : Boolean, cs)
...
elsif :: Env -> (A.Expression, A.Command) -> D (Expression, Command)
elsif env (e, c) = do
...
optElse env (Nothing) = return (Nothing)
optElse env (Just c)  = do
                      c' <- chkCmd env c
                      return (Just c')
...
infTpExp env (A.ExpCond {A.ecCond = e, A.ecTrue = et, A.ecFalse = ef, A.expSrcPos = sp}) = do
    ...
    require (t' == t'') (srcPos e) ("Conditional expression arguments type are not valid, they may be references type")
    return (t', ExpCond {ecCond = e', ecTrue = et', ecFalse = ef', expType = t', expSrcPos = sp})
