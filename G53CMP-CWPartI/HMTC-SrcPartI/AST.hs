...
data Command
    ...
    | CmdIf {
	  ...
	  ciElseIf  :: [(Expression, Command)], -- ^ ElseIf-Seq
    ...
    | CmdRepeat {
	  ...
	  crUntil   :: Expression,	-- ^ Until-Condition
	  cmdSrcPos :: SrcPos
      }
...
data Expression
   ...
   | ExpLitChar {
    ...
    -- | Cava conditional expressions
    | ExpcjCond {
          ecjCond   :: Expression,	-- ^ Condition
	  ecjTrue   :: Expression,	-- ^ If condition is True
	  ecjFalse  :: Expression,	-- ^ If condition is False
	  expSrcPos :: SrcPos
      }
